...
( function () {
	...
	var KEY = 'dreamrs-scheme';
	var root = document.documentElement;
	var strings = window.dreamrsScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'dreamrs-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'dreamrs-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.dreamrs-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'dreamrs-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
