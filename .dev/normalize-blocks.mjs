...
// DREAMRS_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.DREAMRS_PATTERNS
	? resolve( process.env.DREAMRS_PATTERNS )
...
// `<?php ... ?>` -> `DRPHP0`, and back again after the round trip.
...
function restore( content, snippets ) {
	return content.replace( /DRPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
