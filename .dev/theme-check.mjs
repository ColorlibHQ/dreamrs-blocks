 ...
 * Playground for this mounts the build output as `dreamrs` and installs the
 * plugin from the directory:
 *
 *   bash .dev/build-zip.sh "$SCRATCH/dreamrs-build"
 *   npx -y @wp-playground/cli@3.1.54 server --port=9493 --php=8.3 --wp=latest \
 *     --mount-before-install="$SCRATCH/dreamrs-build/dreamrs:/wordpress/wp-content/themes/dreamrs" \
 *     --blueprint=.dev/blueprint-themecheck.json --login
 *   WP_URL=http://127.0.0.1:9493 node .dev/theme-check.mjs
 *
 * @package Dreamrs
 */
...
const theme = process.env.THEME || 'dreamrs';
