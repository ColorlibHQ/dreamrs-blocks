 ...
 * nothing else — and a booking form's inputs were once 34px wider than their
 ...
 *
 *   WP_URL=https://example.com DREAMRS_PATHS=/,/contact/ node .dev/overflow-check.mjs
 *
 * @package Dreamrs
 */
...
const paths = ( process.env.DREAMRS_PATHS || '/,/about/,/services/,/projects/,/apartments/,/blog/,/contact/' ).split( ',' ).filter( Boolean );
const widths = ( process.env.DREAMRS_WIDTHS || '1400,1024,768,390' ).split( ',' ).map( Number );
