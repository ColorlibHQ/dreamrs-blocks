 ...
 *
 *   WP_URL=... DREAMRS_URL=/some-page/ node .dev/contrast-rendered.mjs
 *   DREAMRS_DARK=1 WP_URL=... DREAMRS_URL=/ node .dev/contrast-rendered.mjs
 *   DREAMRS_PALETTE=colors-8-onyx WP_URL=... node .dev/contrast-rendered.mjs
 *
 * @package Dreamrs
 */
...
const path = process.env.DREAMRS_URL || '/';
...
const response = await page.goto( site + path, { waitUntil: 'domcontentloaded', timeout: 90000 } );
...
// DREAMRS_DARK=1 measures the same page with dark mode on, which is where the
// palette is lifted rather than replaced and is the likeliest place for a
// pairing to fall under AA.
if ( process.env.DREAMRS_DARK ) {
	await page.evaluate( () => {
		document.documentElement.classList.add( 'dreamrs-dark' );
	} );
	await page.waitForTimeout( 400 );
}

// Text that only appears on hover (an apartment's details over its photograph)
// is visibility:hidden until then, and hidden text is skipped -- which would
// mean it was never measured at all. Show it, in its hovered state.
await page.addStyleTag( { content: '.dreamrs-apartment>.dreamrs-apartment__info{visibility:visible!important;opacity:1!important;transform:none!important;transition:none!important}.dreamrs-apartment::before{opacity:1!important;transition:none!important}' } );
await page.waitForTimeout( 200 );

// DREAMRS_PALETTE=colors-7-midnight measures under another style variation.
await applyPalette( page, process.env.DREAMRS_PALETTE, !! process.env.DREAMRS_DARK );
...
const candidates = await page.evaluate( () => {
	...
	document.querySelectorAll( 'h1,h2,h3,h4,h5,h6,p,li,a,dt,dd,summary,label,button,span' ).forEach( ( el, i ) => {
		...
		el.setAttribute( 'data-dr-cr', String( i ) );
		out.push( {
			i,
...
for ( const c of candidates ) {
	...
	onPhotos++;
	const handle = await page.$( `[data-dr-cr="${ c.i }"]` );
