 ...
 *
 *   WP_URL=https://colorlibhub.com/dreamrs node .dev/alignment-check.mjs
 *   DREAMRS_PATHS=/,/contact/ WP_URL=… node .dev/alignment-check.mjs
 */
...
for ( const width of widths ) {
	...
	for ( const path of paths ) {
		...
		const result = await page.evaluate( () => {
			const off = [];
			let n = 0;
			// A constrained layout justified left or right puts its children to one
			// side on purpose (the hero's copy sits on the left, as the template's
			// does); only a centred one promises centring.
			document.querySelectorAll( '.is-layout-constrained:not(.is-content-justification-left):not(.is-content-justification-right)' ).forEach( ( box ) => {
