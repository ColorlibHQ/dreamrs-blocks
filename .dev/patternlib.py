#!/usr/bin/env python3
"""Block markup helpers for the Dreamrs pattern generator.
...
- Photographs are the theme's own webp files, addressed through
  get_theme_file_uri() so a child theme can replace any of them.
...
def buttons(items, align=None, gap=None, nowrap=False, extra_class=None):
    ...
    inner = "\n".join(items)
    cls = classes("wp-block-buttons", extra_class)
    return '<!-- wp:buttons%s -->\n<div class="%s">%s</div>\n<!-- /wp:buttons -->' % (
        attrs(data), cls, "\n" + inner + "\n"
    )
...
def separator(style=None, align=None, extra_class=None):
    data = {}
    cls = ["wp-block-separator", "has-alpha-channel-opacity"]
    if style:
        data["className"] = "is-style-" + style
        cls.append("is-style-" + style)
    if extra_class:
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    data["url"] = "DREAMRS_IMAGE"
    if alt:
        data["alt"] = alt
    data["dimRatio"] = dim
    if preset_gradient:
        data["gradient"] = preset_gradient
    elif gradient:
    ...
    if preset_gradient:
        # Verified by serialising a real cover: a preset gradient adds
        # `wp-block-cover__gradient-background` before the gradient classes.
        span = ('<span aria-hidden="true" class="wp-block-cover__background has-background-dim-%d '
                'has-background-dim wp-block-cover__gradient-background has-background-gradient '
                'has-%s-gradient-background"></span>' % (10 * round(dim / 10), preset_gradient))
    elif gradient:
    ...
    url = "<?php echo esc_url( get_theme_file_uri( 'assets/images/%s.webp' ) ); ?>" % slug
    attr_json = attrs(data).replace('"DREAMRS_IMAGE"', '"%s"' % url)

    return (
        '<!-- wp:cover%s -->\n'
        '<div class="%s"%s>'
        '<img class="wp-block-cover__image-background" alt="%s" src="%s" data-object-fit="cover"/>'
        '%s'
        '<div class="wp-block-cover__inner-container">\n%s\n</div></div>\n'
        '<!-- /wp:cover -->' % (attr_json, classes(*cls), style_attr, alt, url, span, inner)
    )


def list_block(items, style=None, extra_class=None, item_classes=None):
    """A core/list whose items may each carry a class (an icon, for instance).

    core/list-item keeps `className` like any block, so a fact such as "3 beds"
    can name its own icon and an editor can change it under Advanced.
    """
    data = {}
    cls = ["wp-block-list"]
    if style:
    ...
    for i, text in enumerate(items):
        item_cls = item_classes[i] if item_classes else None
        if item_cls:
            out.append('<!-- wp:list-item {"className":"%s"} -->\n<li class="%s">%s</li>\n<!-- /wp:list-item -->'
                       % (item_cls, item_cls, text))
        else:
            out.append('<!-- wp:list-item -->\n<li>%s</li>\n<!-- /wp:list-item -->' % text)
    return '<!-- wp:list%s -->\n<ul class="%s">%s</ul>\n<!-- /wp:list -->' % (
        attrs(data), classes(*cls), "".join(out))
