 ...
 * `[dreamrs_contact_form]` survives the round trip because a shortcode is
...
const DREAMRS_SETUP_FLAG = 'dreamrs_front_page_created';
 ...
 */
function dreamrs_starter_pages() {
	return array(
		'home'       => array(
			'title'    => __( 'Home', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-home',
			'template' => 'page-no-title',
		),
		'about'      => array(
			'title'    => __( 'About', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-about',
			'template' => '',
		),
		'services'   => array(
			'title'    => __( 'Services', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-services',
			'template' => '',
		),
		'projects'   => array(
			'title'    => __( 'Projects', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-projects',
			'template' => '',
		),
		'apartments' => array(
			'title'    => __( 'Apartments', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-apartments',
			'template' => '',
		),
		'blog'       => array(
			'title'    => __( 'Blog', 'dreamrs' ),
			'pattern'  => '',
			'template' => '',
		),
		'contact'    => array(
			'title'    => __( 'Contact', 'dreamrs' ),
			'pattern'  => 'dreamrs/page-contact',
			'template' => '',
		),
	);
}
 ...
 * overwrite the first.
 */
function dreamrs_create_front_page() {
	if ( get_option( DREAMRS_SETUP_FLAG ) ) {
		return;
	}

	// Only on a site that has not been built yet: someone activating Dreamrs
	...
	if ( count( $existing ) > 1 ) {
		update_option( DREAMRS_SETUP_FLAG, 'skipped: site already had pages', false );
		return;
	}

	$created = array();

	// The pages are the theme's own pattern markup, not anything a visitor
	// typed. Activation can run in a request where the current user lacks
	// unfiltered_html -- a multisite site admin, or no user at all during a
	// scripted setup -- and kses then strips the contact page's map <iframe>
	// and the inline style that keeps a coloured word's <mark> transparent.
	// Filters are put back straight afterwards, so nothing else is affected.
	$kses = (bool) has_filter( 'content_save_pre', 'wp_filter_post_kses' );
	if ( $kses ) {
		kses_remove_filters();
	}

	foreach ( dreamrs_starter_pages() as $slug => $page ) {
		// Not get_page_by_path(): it also matches attachments, so an image
		// uploaded as contact.jpg would stop the Contact page being built.
		$taken = get_posts(
			array(
				'name'           => $slug,
				'post_type'      => 'page',
		...
		$content = '';
		if ( $page['pattern'] ) {
			$content = dreamrs_pattern_content( $page['pattern'] );
			if ( '' === $content ) {
				continue;
			}
		}
	...
	dreamrs_create_primary_menu( $created );
	...
	if ( $created ) {
		update_option( DREAMRS_SETUP_FLAG, gmdate( 'c' ), false );
	}
}
add_action( 'after_switch_theme', 'dreamrs_create_front_page' );
 ...
 */
function dreamrs_create_front_page_retry() {
	if ( get_option( DREAMRS_SETUP_FLAG ) || ! current_user_can( 'edit_theme_options' ) ) {
		return;
	}

	dreamrs_create_front_page();
}
add_action( 'admin_init', 'dreamrs_create_front_page_retry' );
...
function dreamrs_pattern_content( $name, $seen = array() ) {
	...
	return (string) preg_replace_callback(
		...
		static function ( $matches ) use ( $seen ) {
			...
			$nested = dreamrs_pattern_content( $attributes['slug'], $seen );
 ...
 */
function dreamrs_create_primary_menu( $pages ) {
	if ( ! $pages ) {
		return;
	}

	$order = array( 'home', 'about', 'services', 'projects', 'apartments', 'blog', 'contact' );
	$items = '';
