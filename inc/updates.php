 ...
 * call our filter.
 */
const DREAMRS_UPDATE_HOST     = 'updates.colorlib.com';
const DREAMRS_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/dreamrs.json';
const DREAMRS_UPDATE_CACHE    = 'dreamrs_update_response';
 ...
 * @return bool
 */
function dreamrs_updates_enabled() {
	/**
	 * Filters whether Dreamrs checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'dreamrs_check_for_updates', true );
}
 ...
 * @return array
 */
function dreamrs_update_payload() {
	$payload = array(
		'theme'      => 'dreamrs',
		'version'    => DREAMRS_VERSION,
	 ...
	 */
	return apply_filters( 'dreamrs_update_payload', $payload );
}
 ...
 * @return array|null
 */
function dreamrs_fetch_update() {
	if ( ! dreamrs_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( DREAMRS_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( dreamrs_update_payload(), DREAMRS_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => dreamrs_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( DREAMRS_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( DREAMRS_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function dreamrs_check_update( $update, $theme_data, $theme_stylesheet ) {
	// Recognised by its Update URI rather than its directory name: every
	// Colorlib theme shares this host, and a site may install Dreamrs under
	// another directory (the colorlibhub demo uses themes/dreamrs-blocks).
	if ( $update || empty( $theme_data['UpdateURI'] ) || DREAMRS_UPDATE_ENDPOINT !== $theme_data['UpdateURI'] ) {
		return $update;
	}

	$release = dreamrs_fetch_update();

	if ( ! $release || version_compare( $release['version'], DREAMRS_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => DREAMRS_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/dreamrs/',
...
add_filter( 'update_themes_' . DREAMRS_UPDATE_HOST, 'dreamrs_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function dreamrs_flush_update_cache() {
	delete_site_transient( DREAMRS_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'dreamrs_flush_update_cache' );
add_action( 'after_switch_theme', 'dreamrs_flush_update_cache' );
 ...
 * their server.
 */
function dreamrs_updates_notice() {
	$screen = get_current_screen();

	// Appearance → Themes, where the theme is chosen: Dreamrs has no settings
	...
	$payload = dreamrs_update_payload();

	// Dreamrs ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Dreamrs checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'dreamrs' );
	?>
	<p class="description dreamrs-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( DREAMRS_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>dreamrs_check_for_updates</code>'
		);
		?>
	</p>
	<style>.dreamrs-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'dreamrs_updates_notice' );
 ...
 * @return string
 */
function dreamrs_update_user_agent() {
	return 'Dreamrs/' . DREAMRS_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function dreamrs_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = dreamrs_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'dreamrs_update_request_args', 10, 2 );
