<?php
/**
 * Form styling, for Dreamrs's own form and for whichever plugin a site uses.
 *
 ...
 * in the middle of a Dreamrs page looking like a different website, the theme
 ...
 * @return string[]
 */
function dreamrs_active_form_plugins() {
	$found = array();
 ...
 */
function dreamrs_enqueue_form_styles() {
	// Dreamrs's own contact form is always a possibility, so the stylesheet
...
add_action( 'wp_enqueue_scripts', 'dreamrs_enqueue_form_styles' );

/**
 * Tell the editor about the same stylesheet, so a form block looks right there.
 */
function dreamrs_editor_form_styles() {
	add_editor_style( 'assets/css/forms.css' );
}
add_action( 'after_setup_theme', 'dreamrs_editor_form_styles', 20 );
