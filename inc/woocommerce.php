 ...
 * WooCommerce support.
 *
 * A developer that sells plans, deposits or viewing slots online ends up with
 ...
 */
function dreamrs_woocommerce_setup() {
...
add_action( 'after_setup_theme', 'dreamrs_woocommerce_setup' );
 ...
 */
function dreamrs_woocommerce_styles() {
	if ( ! class_exists( 'WooCommerce' ) ) {
		return;
	}
	 ...
	 */
	if ( ! apply_filters( 'dreamrs_load_woocommerce_styles', $needed ) ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'dreamrs_woocommerce_styles', 20 );
 ...
 * @return int
 */
function dreamrs_woocommerce_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'dreamrs_woocommerce_columns' );
